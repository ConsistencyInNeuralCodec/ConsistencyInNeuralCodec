...
def main(args):
    ...
    OSSREADER = OssReader(sample_rate=16000)

    length_file = open(os.path.join(out_dir, f"wav_length.{rank}.txt"), "wt")
    meeting_count = 0
    # For kaldiio.load_mat(lpath).astype(np.float32) / 32768.0 == librosa.load(lpath, sr=24000)
    for i, (uttid, wav_path) in tqdm(enumerate(local_all_recs), total=len(local_all_recs), ascii=True):
        # OSSREADER.read_kaldiwav(wav_path)
        # array, rate = OSSREADER.read_wav(wav_path)
        array, rate = librosa.load(wav_path, sr=16000)
        if array is None:
            continue
        # lpath = '/home/admin_data/renjun.admin/dataset/libritts/dev-other/116/288045/116_288045_000003_000000.wav'
        # y1 = kaldiio.load_mat(lpath)
        # y2 = librosa.load(lpath, sr=24000)
        # y2 = soundfile.read(lpath)
        # # lpath = '/home/admin_data/renjun.admin/dataset/libritts/dev-other/116/288045/116_288045_000003_000000.wav'
        # # retval = kaldiio.load_mat(lpath)
        # # sy2 = librosa.load(lpath, sr=24000)
