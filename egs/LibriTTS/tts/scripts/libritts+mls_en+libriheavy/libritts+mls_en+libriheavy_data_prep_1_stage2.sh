#! /usr/bin/env bash
# bash /home/admin_data/user/model/uniaudio_admin/FunCodec-Dev/egs/LibriTTS/tts/scripts/libritts+mls_en+libriheavy/libritts+mls_en+libriheavy_data_prep_1_stage2.sh
...
# dumpdir=$dump_dir/$corpus_dir
dumpdir=$dump_dir
mkdir -p ${dumpdir}
for name in "${subset_names[@]}"; do
  echo "Process stage 2: "${name}
  python scripts/data_prep.py trans_to_token $utt_data_root/${name}_text.scp ${dumpdir}/${name}_text ${ttsfrd_rsc_zip_file} ${ttsfrd_config_file} --nj ${cpu_num}
done
...
#   python scripts/data_prep.py codec_enc --bin_dir ../codec --wav_scp $utt_data_root/${name}.scp --out_dir ${dumpdir}/${name}_codec --gpu_devices $gpu_devices --njob $gpu_num --model_dir ${codec_model_dir} --model_name=$inference_model --bit_width ${bit_width}
# done
...
#   python scripts/data_prep.py data_pack ${dumpdir}/${name}_text.scp ${dumpdir}/${name}/text ${dumpdir}/${name}_codec/codecs.txt ${dumpdir}/${name}/codec ${dumpdir}/${name}/codec_shape
# done
