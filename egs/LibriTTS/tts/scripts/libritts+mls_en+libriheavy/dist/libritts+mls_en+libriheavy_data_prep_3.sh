#! /usr/bin/env bash
# bash /home/admin_data/user/model/uniaudio_admin/FunCodec-Dev/egs/LibriTTS/tts/scripts/libritts+mls_en+libriheavy/dist/libritts+mls_en+libriheavy_data_prep_3.sh
...
dump_dir=${dump_root}/16rvq/encodec_16k_n16_ds320_largev8_phase_aug_consistent_0.2_quant_in_10.0
...
# utt_data_root=/home/admin_data/user/model/uniaudio_admin/FunCodec-Dev/egs/dataset/LibriTTS
mkdir -p ${tokens_dir}
...
for name in "${subset_names[@]}"; do
  ...
  python scripts/data_prep.py data_pack_for_scp --data_root ${dump_dir}/${name}_codec/logdir --output_dir ${dump_dir}/${name} --scp_name dist --codebook_size 1024 --only_scp_pack
done
