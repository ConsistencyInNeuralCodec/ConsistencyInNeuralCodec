#! /usr/bin/env bash
# bash /home/admin_data/user/model/uniaudio_admin/FunCodec-Dev/egs/LibriTTS/tts/scripts/libritts+mls_en+libriheavy/phase_aug/kernel_size=1.sh
...
codec_model_root=/home/admin_data/user/checkpoints/funcodec_consistency_encodec_LibriTTS+MLS_en/phase_aug_weight/exp
...
# codec_model_name=encoder_16k_n16_ds320_largev8_phase_aug_encodec
codec_model_name=kernel_size=1_encodec

# dump_dir=${dump_root}/${codec_model_name}
dump_dir=${dump_root}/16rvq/${codec_model_name}
...
for name in "${subset_names[@]}"; do
  ...
  python scripts/data_prep.py codec_enc --bin_dir ../codec --wav_scp $utt_data_root/${name}.scp --out_dir ${dump_dir}/${name}_codec --gpu_devices $gpu_devices --njob 2 --model_dir ${codec_model_dir} --model_name=$inference_model --bit_width ${bit_width}
done
