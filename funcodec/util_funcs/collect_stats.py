...
from funcodec.fileio.datadir_writer import DatadirWriter
from funcodec.fileio.npy_scp import NpyScpWriter
from funcodec.torch_utils.device_funcs import to_device
from funcodec.torch_utils.forward_adaptor import ForwardAdaptor
    ...
    for itr, mode in zip([train_iter, valid_iter], ["train", "valid"]):
        ...
        with DatadirWriter(output_dir / mode) as datadir_writer:
            for iiter, (keys, batch) in enumerate(itr, 1):
                # print(666, batch.keys())
                mel2ph = batch.pop("mel2ph", None)
                # print(777, batch.keys())
