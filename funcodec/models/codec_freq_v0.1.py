...
class FreqCodec(AbsGANESPnetModel):
    ...
    def __init__(
        ...
        logging.info(f"gen_model_list = {self.gen_model_list}")
    ...
    def init_timbre_encoder(self, timbre_strategy: Optional[TimbreStrategy] = None):
        ...
        if self.timbre_strategy is not None:
            ...
            if self.timbre_strategy.merge_embed == "cross_attention":
                self.multihead_attn = nn.MultiheadAttention(
                    self.quantizer.code_dim, num_heads=max(1, self.quantizer.code_dim // 64), dropout=0.1, batch_first=True,
        ...
        if self.timbre_strategy is not None and self.timbre_strategy.frame_contrastive_encoder_config is not None:
            self.frame_contrastive_encoder = FrameContrastiveEncoder(config=self.timbre_strategy.frame_contrastive_encoder_config)
    ...
    def forward_timbre_encoder(
        ...
        if self.timbre_strategy is not None and self.timbre_strategy.timbre_encoder_config is not None:
            if self.timbre_strategy.timbre_encoder_config.input_type == "wav":
                ...
                padding_mask = lengths_to_padding_mask(speech_lengths, max_lens=spectrogram.shape[1])
    ...
    def forward_frame_contrastive_encoder(
        ...
        perturbed_speech: Optional[torch.FloatTensor] = None,
        ...
        **kwargs
        ...
        # print(666, speech.sum(), perturbed_speech.sum())
        if encoder_features is None:
            encoded_frames = self._encode(speech)
            encoder_features = encoded_frames[0].emb
        if encoder_perturbed_features is None:
            encoded_perturbed_frames = self._encode(perturbed_speech)
            encoder_perturbed_features = encoded_perturbed_frames[0].emb
    ...
    def _forward_generator(
        ...
        if disturb_speech:
            ...
            disturbed_speech = praat_output.snd.unsqueeze(1)
            encoded_frames = self._encode(disturbed_speech) # offset list: [offset1]
            ...
            frames = [(frame.emb, frame.scale, frame.transformed_speech) for frame in encoded_frames]
            praat_output = None
        if self.frame_contrastive_encoder is not None and not self.frame_contrastive_encoder.config.skip:
            frame_contrastive_encoder_output = self.forward_frame_contrastive_encoder(
                speech=orig_speech,
                perturbed_speech=disturbed_speech,
                speech_lengths=speech_lengths,
                encoder_features=None,
                encoder_perturbed_features=encoded_frames[0].emb,
                feature_lengths=feature_lengths,
                mel2ph=mel2ph,
            )
        else:
