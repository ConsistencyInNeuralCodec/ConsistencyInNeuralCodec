...
from funcodec.modules.streaming_utils.utils import sequence_mask
...
class CifPredictor(nn.Module):
    ...
    def forward(self, hidden, target_label=None, mask=None, ignore_id=-1, mask_chunk_predictor=None,
        ...
        if target_length is not None:
            alphas = alphas * (target_length / token_num)[:, None].repeat(1, alphas.size(1))
...
class CifPredictorV3(nn.Module):
    def __init__(self,
        ...
        if self.upsample_type == 'cnn':
            ...
            self.upsample_cnn = nn.ConvTranspose1d(idim, idim, self.upsample_times, self.upsample_times)
            from funcodec.models.encoder.transformer_encoder import EncoderLayer as TransformerEncoderLayer
            from funcodec.modules.attention import MultiHeadedAttention
            from funcodec.modules.positionwise_feed_forward import PositionwiseFeedForward
