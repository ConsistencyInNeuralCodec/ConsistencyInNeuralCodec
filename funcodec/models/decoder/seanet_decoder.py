...
class SEANetResnetBlock(nn.Module):
    ...
    def __init__(self, dim: int, kernel_sizes: tp.List[int] = [3, 1], dilations: tp.List[int] = [1, 1],
        ...
        hidden = dim // compress
        block = []
        additional_input_available_list = []
        for i, (kernel_size, dilation) in enumerate(zip(kernel_sizes, dilations)): # this is always length 2
            ...
            additional_input_available_list += [False, True]
        ...
        if true_skip:
            ...
            self.shortcut = SConv1d(dim, dim, kernel_size=1, norm=norm, norm_kwargs=norm_params,
                                    causal=causal, pad_mode=pad_mode)
        assert len(self.block) == len(additional_input_available_list)
        self.additional_input_available_list = additional_input_available_list
    ...
    def _block(self, x, **kwargs):
        for i, layer in enumerate(self.block):
            if self.additional_input_available_list[i]:
                x = layer(x, **kwargs)
            else:
                x = layer(x)
        return x
    ...
    def forward(self, x, **kwargs):
        if self.true_skip:
            ...
            return self.shortcut(x, **kwargs) + self._block(x, **kwargs) # This is simply the sum of two tensors of the same size
...
class SEANetDecoder(nn.Module):
    ...
    def __init__(self, input_size: int = 128, channels: int = 1, n_filters: int = 32, n_residual_layers: int = 1,
        ...
        additional_input_available_list = [True]
        ...
        if final_activation is not None: # This is always None
            ...
            additional_input_available_list += [False]
        self.model = nn.Sequential(*model)
        assert len(self.model) == len(additional_input_available_list)
        self.additional_input_available_list = additional_input_available_list
    
    def remove_weight_norm(self):
        """Remove weight normalization from the convolutional layers."""
        for module in self.model:
            if isinstance(module, SEANetResnetBlock) or isinstance(module, SConv1d) or isinstance(module, SConvTranspose1d):
                module.remove_weight_norm()
    ...
    def forward(self, z, **kwargs):
        # z in (B, T, C)
        # y = self.model(z.permute(0, 2, 1), **kwargs)
        y = z.permute(0, 2, 1)
        for i, layer in enumerate(self.model):
            # print(self.additional_input_available_list[i], type(layer))
            if self.additional_input_available_list[i]:
                y = layer(y, **kwargs)
            else:
                y = layer(y)
...
class SEANetResnetBlock2d(nn.Module):
    ...
    def forward(self, x, **kwargs):
        if self.true_skip:
            ...
            return self.shortcut(x, **kwargs) + self.block(x, **kwargs) # This is simply the sum of two tensors of the same size
...
class SEANetDecoder2d(nn.Module):
    ...
    def remove_weight_norm(self):
        """Remove weight normalization from the convolutional layers."""
        for module in self.model:
            if isinstance(module, SEANetResnetBlock2d) or isinstance(module, SConv2d) or isinstance(module, SConvTranspose2d):
                module.remove_weight_norm()
