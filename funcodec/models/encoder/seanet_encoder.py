...
@dataclass
class SEANetEncoderOutput(ModelOutput):
...
class SEANetResnetBlock(nn.Module):
    ...
    def remove_weight_norm(self):
        ...
        if isinstance(self.shortcut, SConv1d):
            self.shortcut.remove_weight_norm()
...
class SEANetEncoder(nn.Module):
    ...
    def __init__(self, input_size: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
                 ...
                 add_snake_activation=False, last_norm_config='v1'):
        super().__init__()
        ...
        if last_norm_config == 'v1':
            last_norm = norm
            last_norm_params = norm_params
        elif last_norm_config == 'v2':
            print('Using normalize but without affine transformation')
            last_norm = norm
            last_norm_params = {'affine': False}
        elif last_norm_config == 'v3':
            print('do not apply normalized before quantization')
            last_norm = 'none'
            last_norm_params = norm_params
        else:
        ...
        model += [
            # act(**activation_params),
            get_activation(activation, **{**activation_params, "channels": mult * n_filters}),
            SConv1d(mult * n_filters, dimension, last_kernel_size, norm=last_norm, norm_kwargs=last_norm_params,
                    causal=causal, pad_mode=pad_mode)
        ]
    ...
    def remove_weight_norm(self):
        ...
        for module in self.model:
            if isinstance(module, SEANetResnetBlock) or isinstance(module, SConv1d) or isinstance(module, SConv2d):
                module.remove_weight_norm()
...
class SEANetResnetBlock2d(nn.Module):
    ...
    def remove_weight_norm(self):
        ...
        if isinstance(self.shortcut, SConv2d):
            self.shortcut.remove_weight_norm()
...
class SEANetEncoder2d(nn.Module):
    ...
    """
    def __init__(
        self, input_size: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
        ...
        last_norm_config='v1',
        apply_additional_block: bool = False,
    ):
        super().__init__()
        self.channels = input_size
        self.dimension = dimension
        ...
        # transformer_encoder_input_layer = "linear"
        # transformer_encoder_input_layer = "conv2d"
        transformer_encoder_input_layer = None
        ...
        if seq_model == 'lstm':
            ...
            model += [TransformerEncoder(mult * n_filters,
                                         output_size=mult * n_filters,
                                         num_blocks=seq_layer_num,
                                         input_layer=transformer_encoder_input_layer,
                                         causal_mode="causal" if causal else "None",
                                         skip=res_seq)]
        else:
            pass
        ...
        if last_norm_config == 'v1':
            ...
            last_norm = None
            last_norm_params = None
            assert False, 'not implemented error'

        print('last norm:', last_norm)
        print('last norm params:', last_norm_params)
        ...
        model += [
            ...
            SConv1d(mult * n_filters, dimension,
                    ...
                    # norm=norm, norm_kwargs=norm_params,
                    norm=last_norm, norm_kwargs=last_norm_params,
                    causal=causal, pad_mode=pad_mode)
        ]

        if apply_additional_block:
            # seq_model = "transformer"
            if seq_model == 'lstm':
                model += [SLSTM(dimension, num_layers=seq_layer_num, skip=res_seq)]
            elif seq_model == "transformer":
                from funcodec.modules.normed_modules.transformer import TransformerEncoder
                model += [TransformerEncoder(dimension,
    ...
    def remove_weight_norm(self):
        """Remove weight normalization from the convolutional layers."""
        for module in self.model:
            if isinstance(module, SEANetResnetBlock2d) or isinstance(module, SConv2d):
                module.remove_weight_norm()
    ...
    def forward(self, x) -> SEANetEncoderOutput:
        if x.dim() == 3:
            x = x.unsqueeze(1)
        ...
        for i, layer in enumerate(self.model):
            ...
            # print(i, hidden_state.shape, type(layer))
            hidden_states.append(hidden_state)
        return SEANetEncoderOutput(
            last_hidden_state=hidden_state.permute(0, 2, 1),
