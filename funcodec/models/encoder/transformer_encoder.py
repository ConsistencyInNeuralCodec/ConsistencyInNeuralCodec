...
from funcodec.models.encoder.abs_encoder import AbsAdaptiveEncoder
...
from funcodec.modules.layer_norm import AdaptiveLayerNorm
...
class AdaptiveEncoderLayer(nn.Module):
    ...
    Args:
        ...
        concat_after (bool): Whether to concat attention layer's input and output.
            ...
            i.e. x -> x + linear(concat(x, att(x)))
            if False, no additional linear will be applied. i.e. x -> x + att(x)
        stochastic_depth_rate (float): Probability to skip this layer.
            During training, the layer may skip residual computation and return input
            as-is with given probability.
        cond_size (int): Condition dimension.
    """
    ...
    def __init__(
        ...
        """Construct an AdaptiveEncoderLayer object."""
        super(AdaptiveEncoderLayer, self).__init__()
        self.self_attn = self_attn
        self.feed_forward = feed_forward
        self.norm1 = LayerNorm(size) if cond_size == -1 else AdaptiveLayerNorm(cond_size, size)
        self.norm2 = LayerNorm(size) if cond_size == -1 else AdaptiveLayerNorm(cond_size, size)
        self.dropout = nn.Dropout(dropout_rate)
        self.size = size
        self.normalize_before = normalize_before
        self.concat_after = concat_after
    ...
    def forward(self, x, g=None, key_mask=None, attn_mask=None):
        """Compute encoded features.

        Args:
            x (torch.Tensor): Input tensor (#batch, time1, size).
            g (torch.Tensor): Condition tensor (#batch, cond_size).
            key_mask (torch.Tensor): Mask tensor for the input (#batch, time1).
            attn_mask (torch.Tensor): Mask tensor for the attention (#batch, time1, time2). time1 == time2

        Returns:
            torch.Tensor: Output tensor (#batch, time1, size).
            torch.Tensor: Condition tensor (#batch, cond_size).
            torch.Tensor: Key Mask tensor (#batch, time1).
            torch.Tensor: Attention Mask tensor (#batch, time1, time2).
        ...
        if self.concat_after:
            self_attn_output = self.self_attn(x_q, x, x, attn_mask)
            x_concat = torch.cat((x, self_attn_output.attn_output), dim=-1)
            x = residual + stoch_layer_coeff * self.concat_linear(x_concat)
        else:
            self_attn_output = self.self_attn(x_q, x, x, attn_mask)
            x = residual + stoch_layer_coeff * self.dropout(
                self_attn_output.attn_output
            )
        if not self.normalize_before:
            if isinstance(self.norm1, LayerNorm):
                x = self.norm1(x)
...
class TransformerAdaptiveEncoder(AbsAdaptiveEncoder):
    """Transformer Adaptive encoder module.

    Args:
        input_size: input dim
        cond_size: condition dim
        output_size: dimension of attention
        attention_heads: the number of heads of multi head attention
        linear_units: the number of units of position-wise feed forward
        num_blocks: the number of decoder blocks
    ...
    def __init__(
            ...
            positional_dropout_rate: float = 0.1,
            attention_dropout_rate: float = 0.0,
            input_layer: Optional[str] = "embed",
            pos_enc_class=PositionalEncoding,
            normalize_before: bool = True,
            concat_after: bool = False,
            positionwise_layer_type: str = "linear",
        ...
        if self.normalize_before:
            self.after_norm = LayerNorm(output_size) if cond_size == -1 else AdaptiveLayerNorm(cond_size, output_size)
    ...
    def forward(
        ...
        Args:
            xs_pad: input tensor (#batch, time1, input_size)
            conditions: condition tensor (#batch, cond_size)
            padding_masks: padding masks tensor (#batch, time1) 1 for mask
            attn_masks (torch.Tensor): Mask tensor for the attention (#batch, time1, time2). time1 == time2 1 for mask

        Returns:
            torch.Tensor: Output tensor (#batch, time1, output_size).
        """
        bs, seqlen, _ = xs_pad.size()
        if padding_masks is None:
            padding_masks = torch.zeros((bs, seqlen), dtype=torch.bool).to(xs_pad.device)
        if attn_masks is None:
            attn_masks = torch.zeros((bs, seqlen, seqlen), dtype=torch.bool).to(xs_pad.device)

        key_masks = (~padding_masks[:, None, :]).to(xs_pad.device)
        ...
        # xs_pad, conditions, key_masks, attn_masks = self.encoders(xs_pad, conditions, key_masks, ~attn_masks)
        layer_outputs = []
        for layer_id, layer in enumerate(self.encoders):
            layer_output = layer(xs_pad, conditions, key_masks, ~attn_masks)
            # print(type(layer_output), layer_output.keys())
            # xs_pad, conditions, _, _ = layer_output
            # print(layer_id, len(self.encoders), xs_pad.sum())
            xs_pad, conditions = layer_output["last_hidden_state"], layer_output.get("condition", None)
            layer_outputs.append(layer_output)

        if self.normalize_before:
            if isinstance(self.after_norm, LayerNorm):
                xs_pad = self.after_norm(xs_pad)
            else:
                xs_pad = self.after_norm(xs_pad, conditions)

        # return xs_pad
        return self.Output(
            last_hidden_state=xs_pad,
