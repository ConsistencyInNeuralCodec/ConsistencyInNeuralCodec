...
class CodecPreprocessor(AbsPreprocessor):
    def __init__(
            self,
            train: bool,
            rir_scp: str = None,
            ...
            speech_volume_normalize: float = None,
            speech_name: str = "speech",
            speech_max_length: int = -1,
            frame_shift: int = 160,
            speech_rms_normalize: bool = False,
            token_type: str = "char",
            token_list: Union[Path, str, Iterable[str]] = None,
            text_name: str = "text",
            get_speech_length_while_speech_max_length: bool = True,
            clip_audio_from_left_side: bool = None,
            clip_ds_rate: int = None,
    ):
        super().__init__(train)
        self.train = train
        self.speech_name = speech_name
        ...
        if token_list is not None:
            if isinstance(token_list, list):
                ...
                for line in open(token_list, "rt"):
                    self.token_list.append(line.strip().split(" ", maxsplit=1)[0])
            logging.info(f"vocab_size of preprocessor: {len(self.token_list)}")
        self.speech_volume_normalize = speech_volume_normalize
        self.speech_rms_normalize = speech_rms_normalize
        self.rir_apply_prob = rir_apply_prob
        self.noise_apply_prob = noise_apply_prob
        self.speech_max_length = speech_max_length
        self.frame_shift = frame_shift
        self.get_speech_length_while_speech_max_length = get_speech_length_while_speech_max_length
        self.clip_audio_from_left_side = clip_audio_from_left_side
        self.clip_ds_rate = clip_ds_rate
    ...
    def _speech_process(
        ...
        if self.speech_name in data:
            ...
            # normalize speech volume
            if self.speech_volume_normalize is not None:
                speech = data[self.speech_name]
                ma = np.max(np.abs(speech))
                data[self.speech_name] = speech * self.speech_volume_normalize / ma
            elif self.speech_rms_normalize:
                speech = data[self.speech_name]
                # RMS: root(mean(square()))
                volume = np.sqrt(np.mean(np.square(speech)))
                scale = 1e-8 + volume
                data[self.speech_name] = speech / scale

            # clip speech to target length
            if self.speech_max_length > 0:
                speech = data[self.speech_name]
                audio_length = len(speech)
                max_length = self.speech_max_length
                if audio_length > self.speech_max_length:
                    max_start = audio_length - max_length
                    if self.clip_audio_from_left_side is None or self.clip_audio_from_left_side == False:
                        start = np.random.randint(0, max_start, (1,))[0]
                    elif self.clip_audio_from_left_side == True:
                        start = 0
                        # print(f"666 clip_audio_from_left_side = {self.clip_audio_from_left_side}")
                    speech = speech[start:start + max_length]
                    if "ppg" in data:
                        ppg = data["ppg"]
                        st, dur = start / self.frame_shift, max_length / self.frame_shift
                    ...
                    if "ppg" in data:
                        pad_len = max_length / self.frame_shift - data["ppg"].shape[0]
                ...
                if self.get_speech_length_while_speech_max_length:
                    # real length of the speech, not max_length
                    data["speech_lengths_2"] = np.array([min(max_length, audio_length)])
                data[self.speech_name] = speech
    ...
    def _text_process(
        ...
        if self.text_name in data and self.token_list is not None:
            text = data[self.text_name]
            if self.token_type == "char":
                text_ints = [self.token_list.index(x) if x in self.token_list
                             else self.token_list.index("<unk>")
                             for x in text.strip().split(" ")]
                data[self.text_name] = np.array(text_ints, dtype=np.int64)
            else:
                raise TypeError(f"Unknown token type {self.token_type}")
