...
from funcodec.schedulers.abs_scheduler import AbsValEpochStepScheduler
...
from funcodec.torch_utils.add_gradient_noise import add_gradient_noise
...
from funcodec.train.reporter import Reporter
    ...
    def run(
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            for scheduler in schedulers:
                if isinstance(scheduler, AbsEpochBatchStepScheduler):
                    scheduler.step_epoch(iepoch - 1)
        ...
        **kwargs
        ...
        for iiter, (_, batch) in enumerate(
            ...
            if iiter % accum_grad == 0:
                has_scaled_adam = False
                for optimizer in optimizers:
                    if isinstance(optimizer, ScaledAdam):
                        has_scaled_adam = True
                        break
                ...
                if scaler is not None:
                    ...
                    for iopt, optimizer in enumerate(optimizers):
                        if optim_idx is not None and iopt != optim_idx:
                            continue
                        if not isinstance(optimizer, ScaledAdam):
                        ...
                        torch.save(
                            {
                                ...
                                if hasattr(to_save_model, "encoder") and hasattr(to_save_model.encoder, "ema") and to_save_model.encoder.ema is not None else None,
                            },
                            buffer,
                        )
