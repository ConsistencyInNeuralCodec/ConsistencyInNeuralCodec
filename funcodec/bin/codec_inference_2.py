...
def disable_modules(codec_model, modules: Sequence[str]) -> Dict:
    stored_modules = OrderedDict()
    for module_name in modules:
        model_module = getattr(codec_model, module_name, None)
        if model_module is not None:
            stored_modules[module_name] = model_module
            setattr(codec_model, module_name, None)
...
class Speech2Token(nn.Module):
    ...
    def __init__(
        ...
        if model_args.encoder_conf.get("norm", None) == "weight_norm":
            model.remove_encoder_weight_norm()
        if model_args.decoder_conf.get("norm", None) == 'weight_norm':
            model.remove_decoder_weight_norm()
        # model.remove_encoder_weight_norm()
        # model.remove_decoder_weight_norm()
        ...
        self.stored_modules = None
        if disabled_modules is not None:
            disabled_modules = disabled_modules.split('+')
            logging.info(f"disabled_modules = {disabled_modules}")
            self.stored_modules = disable_modules(self.model, disabled_modules)
    ...
    def __call__(
        ...
        batch = [speech, speech_lengths]
        exchange_timbre = kwargs.get("exchange_timbre", None)
        without_timbre = kwargs.get("without_timbre", None)
        if ppg is not None:
            ppg = ppg.to(self.device)
            batch = [speech, ppg]
        # need_recon = False
        # print(f"666 run_mod = {run_mod}, need_recon = {need_recon}")
        if run_mod == "inference":
            ret_dict = self.model.inference(*batch, need_recon=need_recon, bit_width=bit_width, use_scale=use_scale, exchange_timbre=exchange_timbre, without_timbre=without_timbre)
        elif run_mod == "encode":
            # ret_dict = self.model.inference_encoding(*batch, need_recon=False, bit_width=bit_width, exchange_timbre=exchange_timbre, without_timbre=without_timbre)
            ret_dict = self.model.inference_encoding(*batch, need_recon=need_recon, bit_width=bit_width, exchange_timbre=exchange_timbre, without_timbre=without_timbre)
        elif run_mod == "encode_wn":
            ret_dict = self.model.inference_encoding_withoutnorm(*batch, need_recon=False, bit_width=bit_width)
            ...
            ret_dict = self.model.inference_decoding(batch[0])
        results = (
            ret_dict["code_indices"],
            ret_dict["code_embeddings"],
            ret_dict["recon_speech"],
            ret_dict["sub_quants"],
            ret_dict.get("dist_list", None),
        )
        return results
...
def inference_modelscope(
    ...
    logging.info(f"kwargs = {kwargs.keys()}")
    logging.info(f"need_dist = {kwargs.get('need_dist', False)}")
    ...
    model_kwargs = dict(
        ...
        bit_width = bit_width,
        disabled_modules=kwargs.get("disabled_modules", None)
    )
    ...
    exchange_timbre = kwargs.get("exchange_timbre", None)
    without_timbre = kwargs.get("without_timbre", None)
    logging.info(f'666 exchange_timbre = {exchange_timbre}')
    logging.info(f'without_timbre = {without_timbre}')

    speech_id2speech_id = {}
    speech_id2wav_fn = {}
    speech_id2speech_id_txt = kwargs.get("speech_id2speech_id", None)
    speech_id2wav_fn_scp = kwargs.get("speech_id2wav_fn", None)
    logging.info(f"speech_id2speech_id_txt = {speech_id2speech_id_txt}")
    logging.info(f"speech_id2wav_fn_scp = {speech_id2wav_fn_scp}")
    if speech_id2speech_id_txt is not None:
        with open(speech_id2speech_id_txt, mode="r+") as f:
            for line in f.readlines():
                line = line.strip()
                src_speech_id, tgt_speech_id = line.split()
                speech_id2speech_id[src_speech_id] = tgt_speech_id
    if speech_id2wav_fn_scp is not None:
        with open(speech_id2wav_fn_scp, mode="r+") as f:
            for line in f.readlines():
                line = line.strip()
                speech_id, wav_fn = line.split()
                speech_id2wav_fn[speech_id] = wav_fn
    ...
    def _forward(
        ...
        loader = GANSpeechCodecTask.build_streaming_iterator(
            ...
            inference=True,
            mel2ph_file=kwargs.get("mel2ph_file", None),
            speaker_id_dir=kwargs.get("speaker_id_dir", None),
        )
        ...
        # code_embedding_writer = None
        # if "need_indices" in kwargs and kwargs["need_indices"]:
        #     code_embedding_writer = open(os.path.join(output_path, "codec_emb.txt"), "wt")
        #     code_embedding_dir = os.path.join(output_path, 'code_embedding')
        #     os.makedirs(code_embedding_dir, exist_ok=True)
        ...
        dist_writer = None
        if "need_dist" in kwargs and kwargs["need_dist"]:
            outfile_path = os.path.join(output_path, "dist")
            dist_writer = kaldiio.WriteHelper("ark,scp,f:{}.ark,{}.scp".format(outfile_path, outfile_path))
        logging.info(f"666 output_path = {output_path}")
        # logging.info(f"in '_forward', need_dist = {kwargs.get('need_dist', None)}")
        ...
        def write_code_embedding(_key, _indices, batch_id=0, length=None):
            if code_embedding_writer is None:
                return
            # n_frame x B x T x H (dim), n_frame is always 1
            to_write = _indices[0][0][batch_id, :length].cpu().numpy()
            write_path = os.path.join(code_embedding_dir, f'{_key}.emb.npy')
            np.save(write_path, to_write)
            code_embedding_writer.write(_key + " " + write_path + "\n")
        ...
        def write_dist(_key, _dist, batch_id=0, length=None):
            if dist_writer is None:
                return

            # n_q x B x T x D
            to_write = _dist[0]
            # T x n_q x D
            to_write = to_write.permute(1, 2, 0, 3)[batch_id][:length]
            to_write = rearrange(to_write, "t ... -> t (...)")
            to_write = to_write.cpu().numpy()
            logging.info(f"666 write_dist: key = {_key}, to_write = {to_write.shape}")

            dist_writer(_key, to_write)

        # for keys, batch in tqdm.tqdm(loader):
        for keys, batch in loader:
            ...
            if exchange_timbre:
                assert _bs <= 2 and batch["speech"].shape[0] <= 2 and batch["speech_lengths"].shape[0] <= 2 
                if _bs == 1:
                    keys = [keys[0], None]
                    batch["speech_lengths"] = torch.cat([batch["speech_lengths"], batch["speech_lengths"]], dim=0)
                    _bs = 2
                src_speech_id = keys[0]
                tgt_speech_id = speech_id2speech_id[src_speech_id]
                keys[1] = tgt_speech_id
                src_speech_data = batch["speech"][0][:batch["speech_lengths"][0]]
                tgt_speech_data, _ = librosa.load(speech_id2wav_fn[tgt_speech_id], sr=sampling_rate)
                tgt_speech_data = torch.from_numpy(tgt_speech_data)
                tgt_speech_data = tgt_speech_data.to(src_speech_data.device)
                tgt_speech_data = tgt_speech_data.type_as(src_speech_data)
                batch["speech_lengths"][1] = tgt_speech_data.shape[0]
                batch["speech"] = pad_sequence([src_speech_data, tgt_speech_data], batch_first=True, padding_value=0.0)
            ...
            # print(666, keys, batch["speech"].shape, batch["speech_lengths"])
            
            # additional inputs
            if "mel2ph" in batch:
                batch["mel2ph"] = [item[0] for item in batch["mel2ph"]]
            if "frame2ph_token_ids" in batch:
                batch["frame2ph_token_ids"] = [item[0] for item in batch["frame2ph_token_ids"]]
            if "speaker_ids" in batch:
                batch["speaker_ids"] = [item[0] for item in batch["speaker_ids"]]

            speech_length = batch.get("speech_lengths")
            ...
            token_id, token_emb, recon_speech, sub_quants, dist_list = my_model(
                **batch, need_recon=kwargs["need_recon"],
            ...
            for i, key in enumerate(keys):
                ...
                if output_path is not None:
                    ...
                    if write_emb_flag and token_emb is not None:
                        write_code_embedding(key, token_emb, batch_id=i, length=codec_len)

                    if sub_quants is not None:
                        write_sub_quants(key, sub_quants, batch_id=i, length=codec_len)

                    if dist_list is not None:
                        write_dist(key, dist_list, batch_id=i, length=codec_len)
                else:
...
def get_parser():
    ...
    group.add_argument(
        "--need_dist",
        type=str2bool,
        help="whether to dump l2 distance",
    )
    group.add_argument(
        "--run_mod",
        type=str,
        choices=["inference", "encode", "decode", "decode_emb", "encode_wn"],
        default="inference",
        help="run mode",
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        help="*_exchange_timbre.txt",
    )
    group.add_argument(
        "--speech_id2wav_fn",
        type=str,
        default=None,
        help="*_exchange_timbre.scp",
    )
    group.add_argument(
        "--without_timbre",
        action="store_true",
...
if __name__ == "__main__":
    print(f"666 codec_inference_2 start")
    main()
