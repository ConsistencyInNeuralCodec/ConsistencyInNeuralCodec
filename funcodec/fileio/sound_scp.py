...
class SoundScpReader(collections.abc.Mapping):
    ...
    def __init__(
        ...
        print(f' ======== Sampling Rate of Dataset is {sampling_rate} ===========')
        ...
        self.OSSREADER = OssReader(sample_rate=sampling_rate)
    ...
    def __getitem__(self, key):
        wav = self.data[key]
        # Always conduct normalize to float32 [-1, 1]
        array, rate = self.OSSREADER.read_wav(wav)
        # if self.normalize:
        #     # soundfile.read normalizes data to [-1,1] if dtype is not given
        #     array, rate = soundfile.read(wav, always_2d=self.always_2d)
        # else:
