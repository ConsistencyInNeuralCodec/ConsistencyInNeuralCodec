...
from funcodec.optimizers.scaled_adam import ScaledAdam
from funcodec.schedulers.eden import Eden
...
class AbsTask(ABC):
    ...
    def build_optimizers(
        ...
        if args.sharded_ddp:
            ...
            if args.optim == "scaledadam":
                parameters_names = []
                parameters_names.append(
                    [name_param_pair[0] for name_param_pair in model.named_parameters()]
                )
                optim = optim_class(params=model.parameters(), parameters_names=parameters_names, **args.optim_conf)
            else:
                optim = optim_class(model.parameters(), **args.optim_conf)

        optimizers = [optim]
        logging.info(f"666 optimizer = {type(optim)}")
        return optimizers
    ...
    def main(cls, args: argparse.Namespace = None, cmd: Sequence[str] = None):
        ...
        if not args.distributed or not args.multiprocessing_distributed:
            ...
            assert args.ngpu > 1 or args.num_worker_count > 1
            cls.main_worker(args)
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        for i, optim in enumerate(optimizers, 1):
            ...
            logging.info(f"666 {i} scheduler = {type(scheduler)}")
        ...
        if not distributed_option.distributed or distributed_option.dist_rank == 0:
            ...
            with (output_dir / "config.yaml").open("w", encoding="utf-8") as f:
                ...
                if args.use_pai:
                    to_save = {key: value for key, value in vars(args).items() if key != "oss_bucket"}
                    values = yaml_no_alias_safe_dump(to_save, None, indent=4, sort_keys=False)
                    args.oss_bucket.put_object(os.path.join(args.output_dir, "config.yaml"), values.encode("utf-8"))
                else:
        ...
        if args.dry_run:
            ...
            collect_stats(
                ...
                train_iter=cls.build_streaming_iterator(
                    ...
                    mel2ph_file=getattr(args, "mel2ph_file", None),
                    speaker_id_dir=getattr(args, "speaker_id_dir", None),
                ),
            ...
            # print(666, getattr(args, "mel2ph_file", None))
        else:
            ...
            if args.dataset_type == "large":
                from funcodec.datasets.large_datasets.build_dataloader import LargeDataLoader
                ...
                logging.info(f"dataset_type = {args.dataset_type}, train_iter_factory = {type(train_iter_factory)}, args.iterator_type = {args.iterator_type}")
            else:
    ...
    def build_sequence_iter_factory(
        ...
        logging.info(f"in 'build_sequence_iter_factory', iter_options.data_path_and_name_and_type = {iter_options.data_path_and_name_and_type}")
